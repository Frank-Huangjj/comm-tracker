...
通信设备厂家进展跟踪数据采集工具。自动采集华为、中兴、爱立信、诺基亚、三星等通信设备厂商的新闻动态、行业资讯、专利和财务数据，通过 LLM 智能分类和摘要生成，输出趋势分析报告。

## 功能概览

- **多源数据采集** — 官网新闻、C114 行业媒体、微信搜狗、CNIPA 专利、巨潮财经（10 个采集器）
- **智能处理管线** — HTML 清洗 → 去重 → 关键词/LLM 增强分类 → LLM 摘要生成
- **Token 用量追踪** — LLM 调用持久化到数据库，每日预算管控
- **摘要自动回填** — 重复采集时自动将 LLM 生成的摘要回写到已有记录
- **趋势分析报告** — 周度/月度自动生成，含厂家活跃度、分类分布、热点关键词
- **数据可视化** — Streamlit 仪表盘，交互式图表
...
| 全量采集包含禁用源 | CLI 取所有注册采集器而非只 enabled 的 | `collect` 命令按 `sources.yaml` 过滤 |

### 运行数据

- **文章总数**: 57 篇，覆盖 5 家厂商、6 个数据源
- **摘要覆盖率**: 61.4%（35/57 篇有 LLM 摘要）
- **LLM 用量**: ~39,000 tokens（分类 20 次 + 摘要 73 次）
- **分类分布**: standard_contribution 37%, industry_news 26%, product_release 25%, tech_dynamic 12%
- **单元测试**: 17/17 通过
